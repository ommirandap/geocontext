...
import postgis_manager
import sqlscripts
from geodict_lib import *

def createCitiesTable(con):
	postgis_manager.executeScripts(con, [sqlscripts.create_cities, sqlscripts.create_geom_cities])

def populateCitiesTable(con):
	reader = csv.reader(open(populate_db_config.source_folder+'cities.csv', 'rb'))
	for row in reader:
		try:
			country_code = row[0]
			city_name = row[1]
		...
		postgis_manager.executeScriptsWithParam(con, [sqlscripts.insert_values_cities], [params])

def createBigCitiesTable(con):
	postgis_manager.executeScripts(con, [sqlscripts.create_big_cities, sqlscripts.create_geom_cities])
...
def populateBigCitiesTable(con):
	...
	for row in reader:
		...
		postgis_manager.executeScriptsWithParam(con, [sqlscripts.insert_values_big_cities], [params])

def createClosestcityFunction(con):
	postgis_manager.executeScripts(con, [sqlscripts.closest_city])

def createGeometryIndex(con):
	postgis_manager.executeScripts(con, [sqlscripts.create_geom_index_cities])

def runVacuum(con):
	old_isolation_level = con.isolation_level
    	con.set_isolation_level(0)
	postgis_manager.executeScripts(con, [ sqlscripts.vacuum_analyze_cities])
